...
# Файл сессии Telethon — после успешного входа Telegram выдаёт сессию,
# которая сохраняется здесь на диске рядом с базой данных, поэтому
# повторно вводить код при перезапуске сервера не нужно.
