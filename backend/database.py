...
if DATABASE_URL.startswith("sqlite"):
    ...
    engine = create_engine(
        ...
        pool_pre_ping=True,
        pool_recycle=300,
        # Сколько реально ждать свободное соединение из пула, прежде чем
        # сдаться -- дефолт SQLAlchemy (30с) на облачном пулере (see
        # ниже) означает, что запрос лучше повиснет на полминуты, чем
        # быстро вернёт понятную ошибку.
        pool_timeout=int(os.environ.get("DB_POOL_TIMEOUT", "10")),
        # Дефолт SQLAlchemy (pool_size=5, max_overflow=10 -> 15 соединений
        # разом) исчерпывается при активном фоновом опросе интерфейса
        # (диалоги/статус/переписка каждые 2.5-3 сек с нескольких вкладок):
        # каждый такой запрос держит соединение с БД занятым, пока сам
        # ждёт ответа от Telegram (секунды), хотя базе в этот момент уже
        # нечего делать.
        #
        # ВАЖНО: если DATABASE_URL смотрит на внешний пулер (например,
        # Supabase pgbouncer -- Session mode на :5432 или Transaction mode
        # на :6543), у него САМОГО есть жёсткий лимит одновременных
        # соединений на весь проект, общий со всеми остальными клиентами
        # этой БД (локальный запуск, другие деплои и т.д.). Слишком большой
        # pool_size/max_overflow здесь не даёт больше пропускной
        # способности, а просто быстрее упирается в лимит пулера --
        # именно так выглядела ошибка "SSL connection has been closed
        # unexpectedly"/не удаётся открыть новое соединение под нагрузкой.
        # Держим пул скромным и настраиваемым через переменные окружения,
        # чтобы подогнать под конкретный план пулера без правки кода.
