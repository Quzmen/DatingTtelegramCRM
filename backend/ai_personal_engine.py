...
Тебе дают свободный текст, который пользователь написал о людях или о себе: заметку, \
описание задачи или дневниковую запись. Твоя задача — достать из него \
структурированные факты: планы, \
...
- related_at: дата И время события в формате ISO 8601 (YYYY-MM-DDTHH:MM:SS), \
если в тексте есть хоть какая-то временная привязка (с учётом того, что \
сегодня {today}). Если указано только время суток без точного часа — \
переведи в конкретное время: утро → 09:00, день → 13:00, вечер → 19:00, \
ночь → 22:00, "после работы" → 19:00. НИКОГДА не возвращай дату без \
времени. Если во всём тексте нет вообще никакой временной привязки \
(ни даты, ни времени суток) — related_at: null.
...
пользователя. Тебе дают агрегированные данные о активности \
пользователя и других людей: когда они завершает задачи, какие задачи откладывают, когда \
создают события. Найди закономерности в поведении.
...
последствия (его время, энергия, прогресс по его \
целям). Ты  советуешь, какой вариант выбрать, и  оцениваешь, как на \
это отреагируют другие люди — раскладываешь последствия действия \
для самого пользователя и людей.
...
async def _call(system_prompt: str, user_content: str, complexity: str = "fast") -> Optional[dict]:
    ...
    try:
        ...
        if response.status_code == 429:
            logger.warning("Personal AI engine: Gemini 429 (лимит запросов) — тихий откат на локальный результат")
            return None
...
def _parse_dt(value) -> Optional[datetime]:
    if not value or not isinstance(value, str):
        return None
    # Модель иногда возвращает голую дату без времени (например, если в
    # тексте было только "вечером"/"после работы" без точного часа) —
    # datetime.fromisoformat() в этом случае молча подставляет 00:00:00,
    # и событие "сегодня вечером" превращается в "сегодня в полночь",
    # т.е. выглядит как уже прошедшее уже с раннего утра того же дня.
    # Дата без явного времени ненадёжна — лучше не указывать related_at
    # вовсе (запись уйдёт в "сейчас"), чем указывать неверное время.
...
_RELATIVE_DAY_WORDS = ("сегодня", "завтра", "послезавтра", "вечером", "утром", "днём", "ночью")
_COMMITMENT_WORDS = (
    "договор", "обещ", "обязу", "созвон", "подтверд", "договорил", "договорюсь",
)
_PLAN_WORDS = ("планир", "собира", "буду ", "намерен")
_PREFERENCE_WORDS = ("нравится", "нравлюсь", "люблю", "предпочита", "желани", "хочу")
_TIME_PATTERN = re.compile(r"\b\d{1,2}[:.]\d{2}\b")


def _local_classify_kind(text: str) -> str:
    """Простая эвристика по ключевым словам — НЕ замена LLM-классификации,
    а разумный минимум для режима AI_PROVIDER=local (без Gemini), чтобы
    "договорюсь встретиться в четверг" не всегда падало в один и тот же
    generic "fact". Ничего не додумывает сверх наличия/отсутствия слов —
    при сомнении возвращает "fact", как и раньше."""
    low = text.lower()
    if any(w in low for w in _COMMITMENT_WORDS):
        return "commitment"
    has_time_marker = (
        any(w in low for w in _WEEKDAY_WORDS)
        or any(w in low for w in _RELATIVE_DAY_WORDS)
        or _TIME_PATTERN.search(low)
    )
    if has_time_marker and any(w in low for w in ("встреч", "созвон", "звонок", "увидимся")):
        return "event"
...
async def extract_memory(text: str, contact_id: Optional[int] = None) -> Dict:
    ...
    Gemini делает простой локальный откат (одна запись с типом,
    угаданным по ключевым словам через _local_classify_kind, а не всегда
    "fact"), а не падает."""
    ...
    # Локальный откат: не пытаемся имитировать полноценное NLP-извлечение
    # дат — честно сохраняем текст одной записью, чтобы пользователь
    # ничего не потерял, и явно помечаем source="local". Тип записи при
    # этом угадываем простой эвристикой по ключевым словам, а не всегда
    # ставим "fact" — так локальный режим остаётся полезным без Gemini.
    stripped = text.strip()
    if not stripped:
        return {"items": [], "source": "local"}
    return {
        "items": [{
            "kind": _local_classify_kind(stripped),
            "title": stripped[:300],
            "details": None,
            "related_at": None,
            "importance": 0.5,
