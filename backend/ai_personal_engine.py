...
def find_time_conflicts(db, user_id: int, related_at: datetime, exclude_id: Optional[int] = None,
    ...
    q = db.query(models.AIMemoryItem).filter(
        models.AIMemoryItem.user_id == user_id,
...
async def analyze_patterns(db, user_id: int) -> List[Dict]:
    ...
    memory_items = db.query(models.AIMemoryItem).filter(models.AIMemoryItem.user_id == user_id).order_by(
