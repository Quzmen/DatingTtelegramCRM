...
async def extract_memory(
    data: schemas.AIMemoryExtractIn, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user),
):
    if data.contact_id is not None and not crud.get_contact(db, current_user.id, data.contact_id):
    ...
    for item in result["items"]:
        row = crud.create_memory_item(
            db, current_user.id, item, source=result["source"], contact_id=data.contact_id, source_text=data.text,
        )
        saved_rows.append(row)
        if item.get("related_at"):
            conflicts.extend(ai_personal_engine.find_time_conflicts(db, current_user.id, item["related_at"], exclude_id=row.id))
...
def list_memory(
    contact_id: Optional[int] = None, only_open: bool = False,
    db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user),
):
    rows = crud.list_memory_items(db, current_user.id, contact_id=contact_id, only_open=only_open)
...
def create_memory_manual(
    data: schemas.AIMemoryItemCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user),
):
    if data.contact_id is not None and not crud.get_contact(db, current_user.id, data.contact_id):
        raise HTTPException(status_code=404, detail="Контакт не найден")
    row = crud.create_memory_item(
...
def update_memory(
    item_id: int, data: schemas.AIMemoryItemUpdate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user),
):
    row = crud.get_memory_item(db, current_user.id, item_id)
    if not row:
...
def delete_memory(item_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    row = crud.get_memory_item(db, current_user.id, item_id)
    if not row:
        raise HTTPException(status_code=404, detail="Запись не найдена")
...
def timeline(db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    now = datetime.utcnow()
    rows = crud.list_memory_items(db, current_user.id, limit=300)
    entries = []
    for r in rows:
        at = r.related_at
...
def get_patterns(db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    return crud.list_patterns(db, current_user.id)


@router.post("/patterns/refresh", response_model=List[schemas.AIPatternOut])
async def refresh_patterns(db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    patterns = await ai_personal_engine.analyze_patterns(db, current_user.id)
    rows = crud.save_patterns(db, current_user.id, patterns)
    return rows
...
async def create_decision(
    data: schemas.AIDecisionIn, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user),
):
    result = await ai_personal_engine.build_decision_tree(data.situation, data.options)
    row = crud.create_decision(db, current_user.id, data.situation, result["options"])
    return _decision_out(row, source=result["source"])


@router.get("/decisions", response_model=List[schemas.AIDecisionOut])
def list_decisions(db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    return [_decision_out(r) for r in crud.list_decisions(db, current_user.id)]


@router.post("/decisions/{decision_id}/choose", response_model=schemas.AIDecisionOut)
def choose_decision(
    decision_id: int, data: schemas.AIDecisionChooseIn, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user),
):
    row = crud.get_decision(db, current_user.id, decision_id)
    if not row:
...
def delete_decision(decision_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    row = crud.get_decision(db, current_user.id, decision_id)
    if not row:
        raise HTTPException(status_code=404, detail="Решение не найдено")
...
def insights(db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    recent_memory = crud.list_memory_items(db, current_user.id, limit=8)
    open_commitments = crud.list_memory_items(db, current_user.id, only_open=True, kind=models.AIMemoryKind.COMMITMENT.value)
    patterns = crud.list_patterns(db, current_user.id)
    decisions = crud.list_decisions(db, current_user.id, limit=5)
    ...
    return schemas.AIInsightsOut(
        memory_count=len(crud.list_memory_items(db, current_user.id, limit=10_000)),
